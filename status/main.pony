...
actor Main
  ...
  new create(env: Env) =>
    ...
    let server = Server(
      ...
      BackendMaker(env) // factory for session-based application backend
...
class val BackendMaker
  let _env: Env
  let _msg: String = "<h1>Hello World!</h1>"
  ...
  fun apply(session: Session): Handler ref^ =>
    BackendHandler(_env, session, _response)
...
class BackendHandler is Handler
  ...
  new ref create(env: Env, session: Session, response: ByteSeqIter) =>
    _session = session
    _response = response
    _logger = StringLogger(Info, env.out,{(a, _) => a.string() })
    _logger.log("BackendHandler created.")

  fun ref apply(request: Request val, request_id: RequestID) =>
    _logger(Info) and _logger.log(request.method().string() + " " + request.uri().string())
